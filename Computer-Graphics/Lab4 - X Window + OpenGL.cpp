...
int initGL(void)
{
  ...
  for (int i = 1; i < 1000; i += 3) {
    //if (alfa < 6.28f) {
      float x = rad*cos(alfa);
      float y = 0.0f;
      float z = -0.5f + rad*sin(alfa); 
      ver_triangle[i] = x;
      ver_triangle[i+1] = y;
      ver_triangle[i+2] = z;
      alfa+=0.1;
	...
	viewMatrix = glm::lookAt(glm::vec3(0.0f, 2.0f, 5.0f), glm::vec3(0.0f, 0.0f, -1.0f), glm::vec3(0.0f, 1.0f, 0.0f)); 
...
int drawGLScene(int counter)
{
    ...
    glDrawArrays(GL_TRIANGLE_FAN, 0, 333); //rysowanie trojkata
