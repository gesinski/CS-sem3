...
; Wyświetlanie znaków * co sekunde
...
obsluga_zegara PROC
    ...
    ; Sprawdzanie licznika opóźnienia
    mov ax, cs:licznik_opoznienia
    inc ax                   ; zwiększ licznik o 1
    mov cs:licznik_opoznienia, ax ; aktualizacja licznika

    cmp ax, 18               ; czy minęło 18 przerwań? (ok. 1 sekunda)
    jb koniec_obslugi        ; jeśli nie, zakończ obsługę przerwania

    ; Wyzerowanie licznika opóźnienia
    mov cs:licznik_opoznienia, 0
    ...
    mov al, byte PTR cs:poprzedni_znak ; kod ASCII
    mov byte PTR es:[bx-2], al
    mov byte PTR es:[bx-1], 00000111B ; kolor

    mov al, byte PTR es:[bx]
    mov byte PTR cs:poprzedni_znak, al
...
koniec_obslugi:
    ...
    wektor8 dd ?
    licznik_opoznienia dw 0 ; zmienna do zliczania opóźnienia   
obsluga_zegara ENDP
;============================================================
